...
import { startFixtureServer, startMcpClient } from "./helpers/harness.mjs";
...
describe("core browser flow (e2e)", { timeout: 120_000 }, () => {
    ...
    before(async () => {
        ...
        await mcp.ok("start_browser", { headless: true, windowSize: { width: 1280, height: 900 } });
    });

    after(async () => {
        await mcp?.call("stop_browser").catch(() => {});
        await mcp?.close();
        await fixtures?.close();
    });

    const open = (page) => mcp.ok("navigate", { url: fixtures.url(page) });

    it("reports the package version and registers exactly the advertised tools", async () => {
        assert.equal(mcp.client.getServerVersion()?.version, pkg.version);
    ...
    it("type clears the field first by default, then click updates the page", async () => {
        ...
        await mcp.ok("type", { selector: byId("name"), text: "Said" });
        await mcp.ok("assert_attribute", { selector: byId("name"), name: "value", expected: "Said" });

        await mcp.ok("click", { selector: byId("greet") });
        const greeting = await mcp.ok("get_text", { selector: byId("greeting") });
    ...
    it("type with clearFirst:false appends to the existing value", async () => {
        ...
        await mcp.ok("type", { selector: byId("name"), text: " Jr", clearFirst: false });
        await mcp.ok("assert_attribute", { selector: byId("name"), name: "value", expected: "prefilled Jr" });
    });

    it("wait_for_element waits for content that appears later", async () => {
        await open("form.html");

        await mcp.ok("click", { selector: byId("load-later") });
        const waited = await mcp.ok("wait_for_element", { selector: byId("late"), visible: true, timeoutMs: 5000 });
        assert.equal(waited.data.displayed, true);
        assert.equal(waited.data.tagName, "p");

        await mcp.ok("assert_text", { selector: byId("late"), expected: "Loaded later", mode: "equals" });
    });

    it("capture_page refs can drive follow-up actions", async () => {
        await open("form.html");

        const snapshot = await mcp.ok("capture_page");
        ...
        await mcp.ok("type", { ref: nameRef, text: "Ref User" });
        await mcp.ok("click", { ref: greetRef });
        const greeting = await mcp.ok("get_text", { selector: byId("greeting") });
    ...
    it("reports failures as tool errors and keeps the session usable", async () => {
        ...
        const missing = await mcp.call("wait_for_element", { selector: byId("does-not-exist"), timeoutMs: 300 });
        assert.equal(missing.isError, true);
        assert.match(missing.text, /Failed waiting for element id=does-not-exist/);

        const mismatch = await mcp.call("assert_text", { selector: { by: "css", value: "h1" }, expected: "Nope", mode: "equals" });
